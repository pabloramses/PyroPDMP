...
class BPS(MCMCKernel):
    ...
    def sample(self, params):
        ...
        if self.ihpp_sampler == 'Exact':
            gradU = z_grads_numpy  # O(d^2) to compute
            ...
            while not finished:
                ...
                # Update z and v
                (y, v) = self.Dynamics(dt, z_numpy - self.z_ref, v)
                ...
                if not finished and dt_switch_proposed < dt_refresh:
                    ...
                    if np.random.rand() * simulated_rate <= switch_rate:
                        ...
                        v = v - 2 * (switch_rate / np.dot(gradU, gradU)) * gradU
                        phaseSpaceNorm = np.sqrt(np.dot(z_numpy - self.z_ref, z_numpy - self.z_ref) + np.dot(v, v))
                        a = -switch_rate
                        b = self.Q * phaseSpaceNorm ** 2 + M2 * phaseSpaceNorm
            ...
            while not finished:
                ...
                if not finished and (refresh or stuck > 10000):
                    # so we refresh
                    if stuck > 10000:
