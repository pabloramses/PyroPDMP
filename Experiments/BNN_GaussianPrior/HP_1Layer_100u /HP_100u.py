...
class BNN(PyroModule):
  def __init__(self):
    ...
    # Weight priors
    self.hidden.lambda_h_w = PyroSample(dist.HalfCauchy(1.).expand([100, 1]).to_event(2))
    self.hidden.tau_h_w = PyroSample(dist.HalfCauchy(1))
    self.hidden.weight = PyroSample(
      lambda hidden: dist.Normal(0., (hidden.lambda_h_w) * (hidden.tau_h_w)).expand([100, 1]).to_event(2))
    # Bias priors
    self.hidden.lambda_h_b = PyroSample(dist.HalfCauchy(1.).expand([100]).to_event(1))
    self.hidden.tau_h_b = PyroSample(dist.HalfCauchy(1))
    self.hidden.bias = PyroSample(
      lambda hidden: dist.Normal(0., (hidden.lambda_h_b) * (hidden.tau_h_b)).expand([100]).to_event(1))

    # Output layer
    self.out = PyroModule[nn.Linear](100, 1)
    # Weight priors
    self.out.lambda_h_w = PyroSample(dist.HalfCauchy(1.).expand([1, 100]).to_event(2))
    self.out.tau_h_w = PyroSample(dist.HalfCauchy(1))
    self.out.weight = PyroSample(
      lambda out: dist.Normal(0., (out.lambda_h_w) * (out.tau_h_w)).expand([1, 100]).to_event(2))
    # Bias priors
...
boom_kernel = Boomerang(bnn, Sigma=np.eye(606), refresh_rate = 100, ihpp_sampler='Corbella')
