...
def subsampling(args, batch_size, shuffle=True):
    num_obs = args[0].shape[0]
    num_batches = int(num_obs / batch_size)
    if args[0].shape[0]%batch_size != 0:
        num_batches = num_obs + 1

    if shuffle:
        indice = random.sample(range(num_obs), num_obs)
        indice = torch.tensor(indice)
        args[0], args[1] = args[0][indice,0].reshape(-1,1), args[1][indice]

    separator = torch.arange(0, num_batches)
    limits = separator * batch_size

    batches = []
    for i in range(len(limits)):
        if i == len(limits)-1:
            batches.append([args[0][limits[i]:], args[1][limits[i]:]])
        else:
            batches.append([args[0][limits[i]:(limits[i+1])], args[1][limits[i]:(limits[i+1])]])

    subsample = torch.randint(0,num_batches, (1,))
    args = batches[subsample]
    return args, batches

def _gen_samples(kernel, warmup_steps, num_samples, hook, chain_id, *args, **kwargs):
    """
    if kernel.subsampling:
        args = list(args)
        args, batches = subsampling(args, kernel.batch_size, kernel.shuffle)
    """
    kernel.setup(warmup_steps, *args, **kwargs)
    """
    AQUI ES DONDE LE PASA LOS DATOS AL KERNEL 
    """
...
class MCMC(AbstractMCMC):
    ...
    def run(self, *args, **kwargs):
        ...
        with optional(
            ...
            """
            if self.kernel.subsampling:
                num_obs = args[0].shape[0]
                num_batches = int(num_obs / self.kernel.batch_size)
                if args[0].shape[0]%self.kernel.batch_size != 0:
                    num_batches = num_obs + 1

                if self.kernel.shuffle:
                    indice = random.sample(range(num_obs), num_obs)
                    indice = torch.tensor(indice)
                    args[0], args[1] = args[0][indice,0].reshape(-1,1), args[1][indice]

                separator = torch.arange(0, num_batches)
                limits = separator * self.kernel.batch_size
